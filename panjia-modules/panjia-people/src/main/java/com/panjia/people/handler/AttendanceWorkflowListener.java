...
/**
 * 考勤月度审批工作流回调监听器（bizType = {@link BizType#ATTENDANCE_APPROVAL}）。
 * <p>
...
public class AttendanceWorkflowListener {
    ...
    @EventListener(condition = "#approvalEvent.bizType == '" + BizType.ATTENDANCE_APPROVAL + "'")
    public void onApprovalEvent(ApprovalEvent approvalEvent) {
        try {
            Long bizId = approvalEvent.getBizId();
            if (bizId == null) {
                log.warn("[考勤工作流] bizId 为空，跳过：{}", approvalEvent);
                return;
            }
            String handler = null;
            String message = null;
            ...
            log.info("[考勤工作流] 回调：bizId={}, status={}, nodeCode={}",
            ...
            log.error("[考勤工作流] 回调处理失败：{}", approvalEvent, e);
        }
    }
}
