...
/**
 * 提成点调整审批工作流回调监听器（bizType = {@link BizType#RATE_ADJUST}）。
 * <p>
...
public class RateAdjustWorkflowListener {
    ...
    public void onApprovalEvent(ApprovalEvent approvalEvent) {
        try {
            ...
            if (bizId == null) {
                log.warn("[提成点调整工作流] bizId 为空，跳过：{}", approvalEvent);
                return;
            }
            String handler = null;
            String message = null;
            ...
            log.info("[提成点调整工作流] 回调：bizId={}, status={}, nodeCode={}",
                bizId, approvalEvent.getStatus(), approvalEvent.getNodeCode());
            rateAdjustService.handleWorkflowEvent(bizId, approvalEvent.getStatus(), handler, message);
        } catch (Exception e) {
            log.error("[提成点调整工作流] 回调处理失败：{}", approvalEvent, e);
        }
    }
}
