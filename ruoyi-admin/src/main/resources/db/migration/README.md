...
**业务侧 vs 运维侧菜单隔离原则**（2026-09-11 菜单收敛教训）：
- 业务侧菜单（如 V100001 的 `2200 业绩管理 → 2201 业绩明细`，`perms=performance:fact:list`）走业务角色（10/11/12/14）。
- 运维侧菜单（如 V140003 的 `2610 业绩明细`，`perms=perf:fact:list`）走超管/总监角色（1/总监）。
- **menu_name 可以同名**（如两个"业绩明细"），但 `perms` 必须不同（前端按 perm 鉴权，后端按 menu_id 控制可见性）。
- **禁止再建"运维侧"中间层节点**：如果业务侧已有同名顶级菜单，运维侧菜单直接挂到业务侧顶级下，不要再造"数据管理 → 业绩管理 → 业绩明细"这种嵌套（与业务菜单重复、用户困惑）。
