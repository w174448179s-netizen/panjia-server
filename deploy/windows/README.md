...
```
deploy/windows/
├── panjia-setup.nsi       NSIS 安装器脚本（编译 EXE 的源文件）
├── build-installer.sh     Mac / Linux 构建脚本（用 Docker 编译 NSIS）
├── build-installer.bat    Windows 构建脚本
├── license.txt            最终用户许可协议
├── scripts/               PowerShell 脚本（核心逻辑）
│   ├── install.ps1        主安装逻辑（Docker检查/安装+部署+激活）
│   ├── uninstall.ps1      卸载逻辑
│   ├── start-app.ps1      启动应用
│   └── stop-app.ps1       停止应用
...
- 或安装后修改 `config\.env` 中的 `PANJIA_LICENSE_SERVER` 配置，然后重启服务
...
### Q: 授权码输入错误，能改吗？
A: 可以。修改 `config\.env` 中的 `PANJIA_AUTH_CODE`，然后重启服务。

### Q: 数据会丢失吗？
A: 数据存储在 `data/` 目录，重启服务不影响。卸载时选择"保留数据"也不会删除。

### Q: 如何更新版本？
A: 运行新版本的安装器，会自动更新镜像并保留数据。
